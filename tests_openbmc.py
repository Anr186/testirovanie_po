...
def test_wrong_data(driver):
    driver.get("https://localhost:2443")
    time.sleep(1)
    driver.find_element(By.ID, "username").send_keys("tank")
    driver.find_element(By.ID, "password").send_keys("T-34-85") 
    driver.find_element(By.XPATH, "//button[@type='submit']").click()
    time.sleep(3)

    assert "login" in driver.current_url, "Как ты зашёл с названием танка вместо пароля?"
